#!/usr/bin/env node
// Fetches one report's Polis results from comhairle and saves them as a snapshot in
// data/<slug>/polis-snapshots/ — the first step of the data pipeline. It reads nothing
// but data/config.json and writes nothing but the snapshot.
//
...
const { ROOT, loadConfig, stepIdFor } = require('./lib/config');
const {
...
const USAGE = `
Fetches one report's Polis results from comhairle and saves them as a validated
snapshot in data/<slug>/polis-snapshots/.

  node scripts/fetch-snapshot.js <slug> [options]

  <slug>          The report to fetch: a key of data/config.json, whose
                  polis.workflowStepId says which poll to fetch.
...
What it writes
  One new snapshot: comhairle's response, minus participant-level data (group
  membership and opinion-map positions), plus a record of where and when it was
  fetched. If the response fails validation, nothing is written.
...
async function fetchReportData(api, step) {
...
(async () => {
  ...
  const step = stepIdFor(loadConfig(), args.report);
